...
async def create_temperature(temperature: schemas.TemperatureCreate, db: CommonDB):
    return await crud.create_temperature(db=db, temperature=temperature)


@router.get("/temperatures/", response_model=list[schemas.Temperature])
async def read_temperatures(common: CommonLimitation, db: CommonDB):
...
async def read_temperatures_by_city(city_id: int, common: CommonLimitation, db: CommonDB):
