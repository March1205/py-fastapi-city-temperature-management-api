...
async def create_city(city: schemas.CityCreate, db: CommonDB):
    return await crud.create_city(db=db, city=city)


@router.get("/cities/", response_model=list[schemas.City])
async def read_cities(common: CommonLimitation, db: CommonDB):
    cities = await crud.get_cities(db, skip=common["skip"], limit=common["limit"])
    return cities


@router.get("/cities/{city_id}", response_model=schemas.City)
async def read_city(city_id: CommonParametersWithId, db: CommonDB):
...
async def delete_city(city_id: CommonParametersWithId, db: CommonDB):
    db_city = await crud.get_city(db, city_id=city_id)
    if db_city is None:
        raise HTTPException(status_code=404, detail="City not found")
